...
serve(async (req) => {
  ...
  try {
    ...
    if (roundNum > 1 && previousTasks && previousTasks.length > 0) {
      difficultyHint = `\n\nЭто раунд ${roundNum}. Ученик оценил задачи предыдущего раунда как "элементарные", значит граница его знаний проходит выше.
Предыдущие задачи, которые оказались слишком лёгкими:
${previousTasks.map((t: string, i: number) => `${i + 1}. ${t}`).join("\n")}

Составь задачи ЗНАЧИТЕЛЬНО СЛОЖНЕЕ предыдущих, чтобы нащупать границу знаний — точку, где ученик начинает испытывать затруднения.`;
    }
...
Составь ровно 3 задачи (не вопросы, а практические упражнения), которые помогут точно определить границу его знаний:
- Задача 1: на уровне чуть выше того, что ученик уверенно знает (проверка нижней границы).
- Задача 2: на уровне его зоны неуверенности (проверка средней границы).
- Задача 3: на уровне чуть выше его зоны неуверенности (проверка верхней границы).
...
Выдавай ответ строго в JSON: {"tasks": ["задача1", "задача2", "задача3"]}`;
